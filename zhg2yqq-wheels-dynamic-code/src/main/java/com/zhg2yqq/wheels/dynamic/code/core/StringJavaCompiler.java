...
public class StringJavaCompiler implements IStringCompiler {
     ...
     */
    @Override
    public CompileResult compiler(String fullClassName, String sourceCode, CalTimeDTO calTime) {
        long compilerTakeTime = -1;
        boolean compileSuccess;
        // 存放编译过程中输出的信息
        DiagnosticCollector<JavaFileObject> diagnosticsCollector = new DiagnosticCollector<>();
        CompileResult result = new CompileResult(fullClassName, diagnosticsCollector);
        if (calTime.isCalCompileTime()) {
            long startTime = System.currentTimeMillis();
            // 编译成字节，并将字节对象放入result
            compileSuccess = compile(sourceCode, result);
            // 设置编译耗时(单位ms)
        ...
        result.setSuccess(compileSuccess);
        return result;
    }
     ...
     * @return 是否编译成功
     */
    private boolean compile(String sourceCode, CompileResult result) {
        // 标准的内容管理器,更换成自己的实现，覆盖部分方法
        StandardJavaFileManager standardFileManager = compiler
                .getStandardFileManager(result.getDiagnosticsCollector(), null, null);
        ...
        JavaCompiler.CompilationTask task = compiler.getTask(null, javaFileManager,
                result.getDiagnosticsCollector(), null, null, Arrays.asList(javaFileObject));
        return task.call();
    }
}
